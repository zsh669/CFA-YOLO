...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # 确保 im0 是一个 PyTorch 张量
            im0_tensor = torch.as_tensor(im0)

            # 创建与 im0_tensor 相同形状的黑色背景张量
            mask_image = torch.zeros_like(im0_tensor)  # 黑色背景

            # 将掩膜转换为白色
            for mask in masks:
                if mask.is_cuda:  # 确保掩膜在CPU上
                    mask = mask.cpu()
                mask_np = mask.numpy().astype(np.uint8)  # 转换为numpy数组
                mask_image[mask_np == 1] = 255  # 将掩膜区域设置为白色

            # 更新 im0 为掩膜效果图
            im0 = mask_image.numpy()  # 转换为NumPy数组
