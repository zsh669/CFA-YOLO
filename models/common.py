...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='Ayolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        # Usage:
        #   PyTorch:              weights = *.pt
...
class C3Res2Net (C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1,c2,n,shortcut,g,e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Res2NetBottle2neck(c_, c_, shortcut) for _ in range(n)))
